...
public class XAnalyzingSuggester extends Lookup {
  ...
  private static final int SEP_LABEL = 0x01; 
  /*  NOCOMMIT
   *  SIMON EDIT: changed from 0xff to 0x01 since we
   *  convert to utf8 in the tokenfilter I think to be
   *  valid UTF-8 and work with all dictionaries we need 
   *  a valid unicode char as a separator 
   */
  ...
  static final Comparator<Pair<Long,BytesRef>> weightComparator = new Comparator<Pair<Long,BytesRef>> () {
    ...
    public static class XBuilder {
        ...
        private int count;
        private Set<BytesRef> seenSurfaceForms = new HashSet<BytesRef>();

        public XBuilder(int maxSurfaceFormsPerAnalyzedForm, boolean hasPayloads) {
            this.outputs = new PairOutputs<Long, BytesRef>(PositiveIntOutputs.getSingleton(true), ByteSequenceOutputs.getSingleton());
        ...
        public void addSurface(BytesRef surface, BytesRef payload, long cost) throws IOException {
            ...
            // grow unbounded (at most 256 entries):
            dedup++;
            if (dedup >= maxSurfaceFormsPerAnalyzedForm || seenSurfaceForms.contains(surface)) {
                // More than maxSurfaceFormsPerAnalyzedForm
                // dups: skip the rest:
                return;
            }
            BytesRef surfaceCopy = BytesRef.deepCopyOf(surface);
            seenSurfaceForms.add(surfaceCopy);
            ...
            if (surfaceFormsAndPayload[count] == null) {
                surfaceFormsAndPayload[count] = new SurfaceFormAndPayload(payloadRef, encodedWeight);
            } else {
                surfaceFormsAndPayload[count].payload = payloadRef;
                surfaceFormsAndPayload[count].cost = encodedWeight;
            }
            count++;
        ...
        public void finishTerm(long defaultWeight) throws IOException {
            ...
            analyzed.length += 2;
            Util.toIntsRef(analyzed, scratchInts);
            ArrayUtil.quickSort(surfaceFormsAndPayload, 0, count);
