    ...
    public Decision canRebalance(ShardRouting shardRouting, RoutingAllocation allocation, boolean explain) {
        ...
        for (int i = 0; i < shards.size(); i++) {
            if (!shards.get(i).active()) {
                return decision(Decision.Type.NO, "[RebalanceOnlyActive] Can only rebalance if all shards are active", explain);
            }
        }
        return decision(Decision.Type.YES, "[RebalanceOnlyActive] Can rebalance, all shards are active", explain);
    }
}
