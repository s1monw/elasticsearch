...
public class Translog extends AbstractIndexShardComponent implements IndexShardComponent, Closeable {
    ...
    public void markCommitted(final long translogId) throws FileNotFoundException {
        try (ReleasableLock lock = writeLock.acquire()) {
            ...
            if (found == false) {
                ...
                for (TranslogReader translog : Iterables.concat(uncommittedTranslogs, Collections.singletonList(current))) {
    ...
    public long newTranslog() throws TranslogException, IOException {
        try (ReleasableLock lock = writeLock.acquire()) {
            ...
            final TranslogWriter newFile = createWriter(old);
            current = newFile;
    ...
    protected TranslogWriter createWriter(@Nullable TranslogWriter reuse) throws IOException {
    ...
    private final class OnCloseRunnable implements Callback<ChannelReference> {
        ...
        public void handle(ChannelReference channelReference) {
            try (ReleasableLock lock = writeLock.acquire()) {
                if (isReferencedTranslogId(channelReference.getId()) == false) {
                    // if the given path is not the current we can safely delete the file since all references are released
                    logger.trace("delete translog file - not referenced and not current anymore {}", channelReference.file());
                    IOUtils.deleteFilesIgnoringExceptions(channelReference.file());
                }
            }
        }
    }
