    ...
    public void testAllDocsQueryString() throws InterruptedException, ExecutionException {
        ...
        for (int i = 0; i < iters; i++) {
            SearchResponse searchResponse = client().prepareSearch("test")
                    .setQuery(queryString("*:*^2.0").boost(2.0f))
                    .execute().actionGet();
            assertNoFailures(searchResponse);
            assertThat(searchResponse.getHits().totalHits(), equalTo(2l));
            assertThat(searchResponse.getHits().getAt(0).score(), equalTo(1.0f));    
            assertThat(searchResponse.getHits().getAt(1).score(), equalTo(1.0f));
        }
    }
