...
public class ClusterStateDiffIT extends ESIntegTestCase {
    public void testClusterStateDiffSerialization() throws Exception {
        DiscoveryNode masterNode = new DiscoveryNode("master", MockTcpTransport.buildFakeLocalAddress(),
                emptyMap(), emptySet(), Version.CURRENT);
        DiscoveryNode otherNode = new DiscoveryNode("other", MockTcpTransport.buildFakeLocalAddress(),
                emptyMap(), emptySet(), Version.CURRENT);
    ...
    private ClusterState.Builder randomNodes(ClusterState clusterState) {
        ...
        for (String nodeId : nodeIds) {
            if (nodeId.startsWith("node-")) {
                nodes.remove(nodeId);
                if (randomBoolean()) {
                    nodes.add(new DiscoveryNode(nodeId, MockTcpTransport.buildFakeLocalAddress(), emptyMap(),
        ...
        for (int i = 0; i < additionalNodeCount; i++) {
            nodes.add(new DiscoveryNode("node-" + randomAsciiOfLength(10), MockTcpTransport.buildFakeLocalAddress(),
